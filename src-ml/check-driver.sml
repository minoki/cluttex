...
datatype driver = DVIPDFMX | DVIPS | DVISVGM | PDFTEX | XETEX | LUATEX | UNKNOWN
fun toString DVIPDFMX = "dvipdfmx"
  | toString DVIPS = "dvips"
  | toString DVISVGM = "dvisvgm"
  | toString PDFTEX = "pdftex"
...
structure expl3 = struct
datatype driver = PDFMODE | DVISVGM | XDVIPDFMX | DVIPDFMX | DVIPS | PDFTEX | LUATEX | XETEX | UNKNOWN
...
fun checkDriver (expected_driver, filelist)
    = let val () = if Message.getVerbosity () >= 1 then
                       ...
                       ()
          val loadedSet = List.foldl (fn ({ kind, path }, set) => if kind = "input" then StringSet.add (set, PathUtil.basename path) else set) StringSet.empty filelist
          fun loaded name = StringSet.member (loadedSet, name)
          val graphics_driver = if loaded "graphics.sty" orelse loaded "color.sty" then
                                    SOME (if loaded "dvipdfmx.def" then
                                              graphics.DVIPDFMX
                                    ...
                                    NONE
          val expl3_driver = if loaded "expl3-code.tex" orelse loaded "expl3.sty" orelse loaded "l3backend-dvips.def" orelse loaded "l3backend-dvipdfmx.def" orelse loaded "l3backend-xdvipdfmx.def" orelse loaded "l3backend-pdfmode.def" orelse loaded "l3backend-pdftex.def" orelse loaded "l3backend-luatex.def" orelse loaded "l3backend-xetex.def" then
                                 SOME (if loaded "l3backend-pdfmode.def" then
                                           expl3.PDFMODE
                                       else if loaded "l3backend-dvisvgm.def" then
                                           expl3.DVISVGM
                                       else if loaded "l3backend-xdvipdfmx.def" then
                                           expl3.XDVIPDFMX
                                       else if loaded "l3backend-dvipdfmx.def" then
                                           expl3.DVIPDFMX
                                       else if loaded "l3backend-dvips.def" then
                                           expl3.DVIPS
                                       else if loaded "l3backend-pdftex.def" then
                                           expl3.PDFTEX
                                       else if loaded "l3backend-luatex.def" then
                                           expl3.LUATEX
                                       else if loaded "l3backend-xetex.def" then
                                           expl3.XETEX
                                       else
                                           expl3.UNKNOWN
                                      )
                                 ...
                                 NONE
          val hyperref_driver = if loaded "hyperref.sty" then
                                    SOME (if loaded "hluatex.def" then
                                              hyperref.LUATEX
                                    ...
                                    NONE
          val xypic_driver = if loaded "xy.tex" then
                                 SOME (if loaded "xypdf.tex" then
          ...
          val () = if Message.getVerbosity () >= 1 then
                       ...
                       ; Message.info ("checkdriver: xypic=" ^ (case xypic_driver of NONE => "not loaded" | SOME d => xypic.toString d))
                       )
                   else
                       ()
          val { graphics = expected_graphics
      ...
      in case graphics_driver of
             NONE => ()
           | SOME d => if d <> expected_graphics then
                           ( Message.diag "The driver option for grahipcs(x)/color is missing or wrong."
                           ; Message.diag ("Consider setting '" ^ graphics.toString expected_graphics ^ "' option.")
                           )
                       else
                           ()
       ; case expl3_driver of
             NONE => ()
           | SOME d => if d <> expected_expl3_old andalso d <> expected_expl3_new then
                           ( Message.diag "The driver option for expl3 is missing or wrong."
                           ; Message.diag ("Consider setting 'driver=" ^ expl3.toString expected_expl3_new ^ "' option when loading expl3.")
                           ; if expected_expl3_old <> expected_expl3_new then
                                 Message.diag ("You might need to instead set 'driver=" ^ expl3.toString expected_expl3_old ^ "' if you are using an older version of expl3.")
                             else
                                 ()
                           )
                       else
       ...
       ; case (hyperref_driver, expected_hyperref) of
             ...
             if actual <> expected then
                 ...
                 ; Message.diag ("Consider setting '" ^ hyperref.toString expected ^ "' option.")
                 )
             else
                 ()
           | _ => ()
       ; case (xypic_driver, expected_xypic) of
             ...
             if actual <> expected then
                 ...
                 ; case (expected_driver, expected) of
                       (DVIPDFMX, _) => Message.diag "Consider setting 'dvipdfmx' option or running \\xyoption{pdf}."
                     | (PDFTEX, _) => Message.diag "Consider setting 'pdftex' option or running \\xyoption{pdf}."
                     | (_, xypic.PDF) => Message.diag "Consider setting 'pdf' package option or running \\xyoption{pdf}."
                     | (_, xypic.DVIPS) => Message.diag "Consider setting 'dvips' option."
                     | (_, xypic.UNKNOWN) => ()
                 )
             else
                 ()
