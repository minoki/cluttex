structure SafeName : sig
              val escapeJobname : string -> string
              val safeInput : { name : string, isPdfTeX : bool } -> string
          end = struct
local
    fun escapeChar #" " = "_"
      | escapeChar c = if Char.isSpace c orelse c = #"\"" orelse c = #"$" orelse c = #"%" orelse c = #"&" orelse c = #"'" orelse c = #"(" orelse c = #")" orelse c = #";" orelse c = #"<" orelse c = #">" orelse c = #"\\" orelse c = #"^" orelse c = #"`" orelse c = #"|" then
...
local
    ...
    fun handleSpaces s = let fun go (s, acc) = if Substring.isEmpty s then
                                                   ...
                                                   let val (a, b) = Substring.splitl (fn c => c <> #" ") s
    ...
    fun handleNonAscii s = let fun go (s, acc) = if Substring.isEmpty s then
                                                     ...
                                                     let val (a, b) = Substring.splitl Char.isAscii s
                                                         val (c, d) = Substring.splitl (Bool.not o Char.isAscii) b
                                                         val c' = if Substring.isEmpty c then
                                                                      c
                                                                  else
...
fun safeInput { name, isPdfTeX } = let val escaped = handleSpaces (String.translate handleSpecialChar name)
                                       val escaped = if isPdfTeX then
                                                         handleNonAscii escaped
                                                     else
                                                         escaped
                                   in if name = escaped then
                                          "\\input\"" ^ name ^ "\""
                                      else
                                          "\\begingroup\\escapechar-1\\let~\\string\\edef\\x{\"" ^ escaped ^ "\" }\\expandafter\\endgroup\\expandafter\\input\\x"
                                   end
end
end;
